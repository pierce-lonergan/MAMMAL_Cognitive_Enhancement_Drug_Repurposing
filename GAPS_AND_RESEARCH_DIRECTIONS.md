...
| PERSEUS persistence-aware engine (F8) | DONE v2.8 | two orthogonal heads (symptomatic class prior vs abstain-by-default persistence); L1 CNS gate + L3 mechanism 3-tier substrate + L4 psychoplastogen window + L5 evidence governor + L0-mismatch guard + prodrug/salt handling. v2.2 GPU persistence-target DTI module: SIZE-MATCHED negatives reveal MAMMAL pKd is MW-confounded; only BCL2 (ablative) survives MW-residualization size-independently. v2.5 L4 psychoplastogen window (Vargas 2023, off-DTI-axis permeability discriminator: serotonin window-NEG vs DMT window-POS despite isoaffine 5-HT2A) lifts recall 0/13->7/13 on a recovered verified positive ledger (neuroplasticity 5/5); first rigorous bidirectional metric: sensitivity 0.54 (Jeffreys CI 0.28-0.78), FPR 0/15, PPV-vs-prior curve. v2.6 L1 Stage-3 efflux-aware conformal logBB regressor (B3DB CC0 + Didziapetris P-gp feature + Mondrian split-conformal + kNN abstain; scaffold R2 0.23, coverage 0.85) replaces the ABSTAIN stub and downgrades P-gp-substrate physchem-PASSes. Honest scope: a CNS-gated abstain-by-default guardrail with measured specificity AND sensitivity on the serotonergic-psychoplastogen class; logBB is a penetration proxy (true unbound Kp,uu off-axis, deferred). v2.7: TrkB-TMD durability site CONFIRMED off-axis (NTRK2 site-split tmd_wedge AUROC 0.23; scripts/112) + grouped-LOMO + label-shift transport (scripts/113). v2.8 four-frontier batch: F1 HDACi pulsed self-maintaining hypothesis tier (Nat Genet 2025); F2 AMPA-PAM + PNN/ECM channels pre-registered negatives CONFIRMED (GRIA1-PAM 0.26, MMP9 0.42; scripts/114) - completes the durability-node negative map (only senolytic-ablative is DTI-recoverable; TrkB-TMD/AMPA-PAM/PNN all off-axis); F3 pluggable ADMET-AI-Pgp efflux hook + documented Kp,uu-spine residual; F4 ledger 16->19 (recall CI 0.27-0.73) | shipped |
...
**v2.1 (bidirectional ground-truth eval, the dimension the reviewer scored lowest):** a
cited persistence-DESIGN ledger (`data/raw/persistence_ground_truth.csv`, 14 scoreable +2
non-structure mAbs - delayed-start / randomized-discontinuation / washout / parallel-group
readouts, labelled by what the trial actually showed) is scored through the engine and each
verdict compared to its trial-design label. **Over-claim rate 0/14** (asserting more
durability than the label supports), `scripts/102_persistence_groundtruth_eval.py`,
`reports/pipeline/perseus_eval_groundtruth_v1.md`. The eval immediately earned its keep: the
first run caught selegiline over-claiming CONTESTED, traced to DATATOP/Sano not being
genuine delayed-start designs (the time-to-levodopa endpoint is confounded by selegiline's
own symptomatic effect, the exact confound delayed-start was invented to remove), so
selegiline was demoted CONTESTED -> tested_negative - leaving rasagiline/ADAGIO and the
fluoxetine plasticity window as the only live threads, exactly the reviewer's point.
...
predictor (sensitivity/PPV need a non-empty positive ledger + PU/leave-one-mechanism-out). Evaluation for the empty-positive class
