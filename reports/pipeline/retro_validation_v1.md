...
**1 of 24 archived hypotheses have a keystone that is now satisfied.**
...
- permanently closed (never swept): **8**
...
| H-R1-power-confound | underpowered | `ledger_rows_at_least(healthy_adult_cognition_ledger.csv, 60)` | healthy_adult_cognition_ledger.csv has 42 row(s) vs required 60 |
...
| underpowered | 6 | yes |
| measured_null | 6 | no |
| unknown_precision | 4 | yes |
| instrument_blind | 2 | yes |
| wrong_endpoint | 2 | yes |
| provenance_failed | 1 | yes |
| mechanism_contradicted | 1 | no |
| measured_harm | 1 | no |
| confounded | 1 | yes |

**16 of 24 entries are revivable.** That is not a claim that 16 hypotheses are alive. It is a claim that for 16 of them the recorded evidence cannot distinguish 'no effect' from 'not measurable by the test that was run', which is a different statement and a much weaker one than the ledgers currently make.
...
Registered predicates: `allosteric_head_beats`, `assay_family_signed_compounds`, `baseline_ability_axis_exists`, `ci_recorded`, `cluster_d_v2g_attribution_measured`, `dose_axis_exists`, `durable_healthy_rows`, `interval_excludes_target`, `ledger_rows_at_least`, `metabolite_prediction_available`, `paired_studies_at_n`, `prodrug_resolution_covers`, `tanimoto_excludes_self`.
