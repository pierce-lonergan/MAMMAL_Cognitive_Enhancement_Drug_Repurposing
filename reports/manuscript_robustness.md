...
The structure-NN score is not merely class membership relabelled: the top-1 Tanimoto neighbour is in the **same** mechanism class for only **6/14 = 43%** of drugs (most structural nearest-neighbours are cross-class), so structure similarity is a genuinely distinct, largely cross-class predictor that still underperforms class aggregation.
...
**Fixed-cutoff hold-out (train ≤ 2014, predict later).** Test AUROC **1.00** over 12 post-2014 drugs (1S/11F); the pre-2015 class track record ranks the lone later success above every subsequent failure.

**Prequential 'as-of' (each drug predicted from strictly-earlier drugs).** Informed AUROC **1.00** (n=19, 8S/11F); full-coverage AUROC 0.96 (n=30); informed coverage 61%. One honest miss (CX-516, 2008, AMPA-PAM) before that class accumulated its failures — the method updating as evidence arrives, not a tautology. Full per-drug trace in `reports/temporal_validation_v1.md` (`scripts/85_temporal_validation.py`).

## 5. Class-taxonomy sensitivity

| Taxonomy | classes | class-LOCO AUROC |
|---|---|---|
| Mechanism class (real) | 11 | **1.000** |
| Coarse (neurotransmitter system) | 4 | 0.615 |
| Random permutation (2000×) | shuffled | 0.460 ± 0.146 |

Granularity-specific: lumping distinct mechanisms (coarse → 0.62) or shuffling labels (random → 0.46, 0/2000 reaching 1.00, perm p = 0.0005) destroys the signal. It lives at the mechanism-class level — the scientific claim.
