...
1. **The PDF is already generated**: `reports/manuscript_class_prognostic.pdf` (typeset via `pandoc manuscript_class_prognostic_biorxiv.md -o manuscript_class_prognostic.pdf --pdf-engine=xelatex --resource-path=.:.. -V geometry:margin=1in` — the figure is embedded and Unicode renders correctly). Regenerate with that command after any manuscript edit.
