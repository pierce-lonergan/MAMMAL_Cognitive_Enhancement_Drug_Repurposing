...
Per `reports/paper-drafts/v8_osf_preregistration.md`:
...
Full report: `reports/pipeline/chemcpa_real_lincs_scale_comparison.md`.
...
Full report: `reports/pipeline/v8_hierarchical_cpg0000_calibration_v1.md`. Posterior
...
| OSF pre-registration | `reports/paper-drafts/v8_osf_preregistration.md` (locked Sec 5.1-5.4 thresholds) | ✅ shipped |
...
2. **On the cpg0000 pilot dataset, where the same 60 compounds were profiled
   ...
   **ICC_cell = 0.018** (cell-line variance fraction) place the empirical
   ...
   (ICC_inter < 0.2). See `reports/pipeline/v8_hierarchical_cpg0000_calibration_v1.md`.
...
7. **OSF.io account + lock** required before production execution (currently `reports/paper-drafts/v8_osf_preregistration.md` is the markdown draft; the actual OSF DOI mint awaits external account setup).
...
Reports:
- `reports/pipeline/v8_chemcpa_smoke_v1.md` (chemCPA architecture training validation)
- `reports/pipeline/v8_gate1_dryrun_v1.md` (Gate 1 mechanism-class recovery validation)
- `reports/wet-lab/wet_lab_shortlist_v10.md` (v10 production deliverable)
- `reports/paper-drafts/v8_osf_preregistration.md` (OSF pre-reg with locked Gate 1-4 thresholds)
...
OSF pre-registration: `reports/paper-drafts/v8_osf_preregistration.md` locks Gate 1-4 thresholds + 30-class mechanism taxonomy + 9+1 nootropic anchor set + I_novel novel-mechanism gate BEFORE unblinding mechanism-class labels.
...
*Generated by `reports/paper-drafts/v8_paper_draft.md`. Companion to V6.A/V6.B/V7 paper drafts + V7/V8 OSF pre-registrations. V8.5 Stage 1 outline; Stage 2 awaits real LINCS+JUMP-CP download.*
