...
> as the immediate v4 fix. See `reports/pipeline/diagnostics_v1.md`,
...
5. Write `reports/pipeline/calibration_report.md` (legible) and `configs/weights_calibrated.yaml` (machine-readable per-target overrides).

### What we found at v1 (this snapshot)

`reports/pipeline/calibration_report.md` summary:
...
Phase D's diff (`scripts/25_v3_fusion_diff.py` → `reports/pipeline/fusion_calibration_diff.md`) shows: Spearman ρ between the two orderings is **+0.994**. Calibration preserves macro-order but materially re-shuffles compounds whose top-target was inverted or weak.
...
* `reports/pipeline/calibration_report.md` — Phase A.7 verdicts per target
* `reports/pipeline/fusion_calibration_diff.md` — Phase D before/after
* `configs/weights.yaml` + `configs/weights_calibrated.yaml` — global + per-target overrides
* `reports/pipeline/chembl_target_id_audit_sqlite.md` — Phase A.6: 21/22 ChEMBL target picks ALIGNED (and the original 13 NO_RECORDS were all REST timeouts, not real misses)
* `reports/pipeline/sqlite_vs_rest_smoke.md` — Phase A.5 gate (19/20 agreement; 1 was a REST 500, no real disagreement)
...
4. **No wet-lab confirmation.** The wet-lab shortlist in `reports/wet-lab/wet_lab_shortlist.md` is a *prioritisation* output, not a recommendation. Every entry has the verdict, the contributing clusters, the gate status, and the ChEMBL backstop verdict so a human can decide whether to send it to assay.
