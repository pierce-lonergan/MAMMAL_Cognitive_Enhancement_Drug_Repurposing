...
A learn-to-rank head that fuses the heterogeneous evidence already in the pipeline — MAMMAL ⊕ Tanimoto-to-actives ⊕ Boltz 3D-affinity ⊕ physicochemistry — trained on ChEMBL and evaluated held-out, **recovers a ranking the foundation model cannot produce**: ρ **+0.514** on the binding-mode benchmark against MAMMAL's −0.24, and **+0.621** under the 297-pair LOTO against MAMMAL's −0.11. The honest lesson for the field: do not use a sequence-only DTI score for within-target ligand ranking at the allosteric/transporter sites that dominate cognition pharmacology.

**What the fusion is not.** Its dominant feature by importance (0.81) is Tanimoto-to-actives, computed against the target's ChEMBL actives at pChEMBL ≥ 8.0 — a set the query compound itself belongs to, so the feature can read the test row's own activity record. 143 of 289 LOTO rows carry it at exactly 1.000. Under LOTO that single feature reaches **+0.765, ahead of the fusion's +0.621**; the paired margin is −0.104, 95% CI [−0.218, +0.032], so what is established is equivalence, not superiority. Structural similarity ranks within-target affinity as well as a fusion containing a 458M-parameter DTI foundation model. A pre-registered robustness analysis (`docs/PREREG_ALLOSTERIC_ROBUSTNESS.md`, results in `reports/pipeline/allosteric_robustness_v1.md`) returns the verdict DEGRADES and shows the dominant leak is the feature, not the train/test split: separating scaffolds costs 0.055, recomputing the similarity self-clean costs a further 0.180. The negative result above is unaffected by any of this; the positive one is bounded by it.
