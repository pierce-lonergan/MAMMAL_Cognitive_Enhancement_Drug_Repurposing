...
- 🔬 **Allosteric learn-to-rank head** (`reports/pipeline/allosteric_ltr_v1.md`, Gap 4): MAMMAL's sequence-only binding is **flat within target** (std 0.01–0.05 across ligands spanning 3 log-units of affinity — it cannot rank binders). On one target it returns the bit-identical score to all three ligands, so there is no ordering to be right or wrong about. A fused head [MAMMAL ⊕ Tanimoto ⊕ Boltz ⊕ physicochemistry] trained on ChEMBL and evaluated **held-out** on the cited allosteric benchmark reaches within-target Spearman ρ **+0.514, against MAMMAL alone at −0.244** — recovering a ranking the foundation model cannot produce. **Disclosed limitation (2026-08-24)**: the fusion's dominant feature is a similarity to the target's ChEMBL actives, a set the query compound belongs to, so it can read the test row's own activity record. Under leave-one-target-out that feature alone reaches **+0.765 against the fusion's +0.621**. Pre-registered robustness analysis, verdict DEGRADES: `reports/pipeline/allosteric_robustness_v1.md`.
