...
| **Gap 4 — allosteric learn-to-rank head** | MAMMAL flat within-target (std 0.01–0.05, bit-identical on HRH3); fused [MAMMAL⊕Tanimoto⊕Boltz⊕physchem] reaches within-target Spearman ρ **+0.514, against MAMMAL alone at −0.244**, held-out on the cited allosteric benchmark (`reports/pipeline/allosteric_ltr_v1.md`). **Caveat (2026-08-24)**: the fusion's dominant feature reads the test row's own ChEMBL activity record; under leave-one-target-out that feature alone reaches **+0.765 against the fusion's +0.621** (`reports/pipeline/allosteric_robustness_v1.md`, pre-registered verdict DEGRADES) | ⚠️ |
...
├── tests/                        503 non-slow pytest cases + 14 slow (33 files)
...
*Last updated 2026-05-29 (Gaps 1–7 shipped + **panel finished to 31 targets with the real MAMMAL DTI head**: v11 grid + disease reframe + retrospective validation + allosteric learn-to-rank + clinician dossiers + external benchmark; V6.A grid 13→31; plus the MH 1-8 sprint suite + chemCPA real-LINCS + V8 cpg0000). The pipeline is end-to-end shipped with a differentiated shortlist on the complete 31-target panel, leakage-audited retrospective + external benchmarks (class track record AUROC 1.00 vs target paradigms ≈ chance), disease-specific shortlists (AD/CIAS/FXS — CIAS now surfaces M1/M4) each validated within-disease, an allosteric learn-to-rank head reaching within-target ρ +0.514 against MAMMAL alone at −0.244 (with a self-matching similarity feature disclosed in `reports/pipeline/allosteric_robustness_v1.md`), clinician-legible GRADE dossiers, and a prospective mechanism-justified repurposing shortlist (Gap 7: CIAS→buspirone/M1, FXS→roflumilast, AD→σ1); what remains is wet-lab validation and OSF/bioRxiv release.*
