...
**Snapshot (post V5 + Tier 2 + Tier 3 + Tier 3b sprint)**: V4 architecture + V5 transition + Tier 2 + Tier 3 + Tier 3b sprints all complete. **6 of 7 Tier-1 items shipped** (Phase B Cluster C remains the only outstanding Tier-1 item, blocked on separate `txgnn_env` venv). **8 of 8 Tier-2 items shipped** (§8.2 DrugComb code-complete with fallback report; activates when network reachable). **13 of 14 Tier-3 items shipped**: §8.0a Pareto, §7.12 conformal, §7.13 scaffold-AL, §8.6 brain-region, §8.12 cron auto-recalib, §8.16 calibrator QC, §7.4 v2 selectivity entropy+PI, §7.15 hierarchical Bayes (shrinkage path; PyMC NUTS code-complete), LambdaMART meta-ranker, §8.14 pocket-routed isotonic, §8.9 ANI-2x stub, plus the hypothesis-validation harness. Production wet-lab handoff: **`reports/wet_lab_shortlist_v6_full.md`** + **`reports/pareto_ranking_v1.md`** (12-compound Pareto front, 10 PASS). **Methodology v2 + §14 Hypothesis Ledger shipped**. **Test coverage**: 67/67 non-slow pytest tests passing (14 new Tier-3b + 14 Tier-3 + 26 V5 + 13 pre-existing). **Hypothesis audit**: 10 PASS / 2 DEGRADE / 0 FAIL (DEGRADEs are §4.9 extrapolation bug + SLC6A3 calibrator drift, both honestly caught). **LambdaMART headline**: NDCG@25 = 0.891 vs baseline 0.774 on held-out targets (+15% lift).
