...
from mammal_repurposing.provenance.report_freshness import (  # noqa: E402
    ...
    load_manifest, scan, undeclared_reports,
)
...
def cmd_check(list_only: bool) -> int:
    ...
    # A report with NO trailer was skipped outright until 2026-09-20, which made omission the
    # cheapest exemption in the repository: no attestation, no named code, and it survived every
    # future commit to the generator. 31 of 115 reports were using it, including
    # l4_window_precision_v1.md (generated by engine/psychoplastogen.py, changed the same week) and
    # perseus_sensitivity_v1.md (whose central claim the L4 demotion reverses). The gate printed
    # "failing 0" the whole time. A report must now DECLARE itself: name a generator, or say it has
    # none. Absence is neither, and absence now fails.
    ...
    if undeclared:
        print(f"  UNDECLARED (invisible to this gate)        {len(undeclared)}")
