...
def main() -> int:
    ...
    # The trailer records the INVOCATION, not just the script. Six reports in
    # reports/pipeline/ come from this file under different flags
    # (--use-tanimoto, --z-normalize, --admet-gates, --no-filter) and every one
    # carried the same bare "Generated by `scripts/27_...py`." line. That makes
    # five of the six unreproducible: nothing on disk records which run produced
    # which file, so the freshness gate's --reproduce cannot replay them and a
    # reader cannot tell selectivity_v2_isotonic from selectivity_v3_isotonic_znorm
    # except by the filename. The gate already parses flags out of a trailer.
    _flags: list[str] = []
    if getattr(args, "use_tanimoto", False):
        _flags.append("--use-tanimoto")
    if getattr(args, "z_normalize", False):
        _flags.append("--z-normalize")
    if getattr(args, "admet_gates", False):
        _flags.append("--admet-gates")
    if getattr(args, "no_filter", False):
        _flags.append("--no-filter")
    if float(getattr(args, "active_pchembl", 8.0)) != 8.0:
        _flags.append(f"--active-pchembl {args.active_pchembl}")
    L.append("Generated by `"
             + " ".join(["scripts/27_v3_selectivity_scoring.py", *_flags]) + "`.")
